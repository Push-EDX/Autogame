...
void Node::parseAttributes() 
{
	...
	while (*ptr && *ptr != '>') 
	{
		...
		// skip unrecognized
        while (*ptr && !isalnum(*ptr) && !_isspace(*ptr)) ++ptr;
...
//				fprintf(stderr, "Trying to find quote: %c\n", quote);
                ...
                while (_isspace(*begin) && begin < end) ++begin;
				const char *trimmed_end = end - 1;
                while (_isspace(*trimmed_end) && trimmed_end >= begin) --trimmed_end;
			...
			else 
			{
                ...
                while (*end && !_isspace(*end) && *end != '>') end++;
				val.assign(ptr, end);
				ptr = end;
			}
